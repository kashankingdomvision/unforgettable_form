...
class AdminController extends Controller
{
    ...
    public function create_role(Request $request){
        if($request->isMethod('post')){
            $this->validate($request, ['name'  => 'required|unique:roles'],['required' => 'Name is Required']);
            $name = $request->name;
            role::create(array(
                'name' => $request->name
            ));
            return Redirect::route('creat-role')->with('success_message', 'Created Successfully');
        }
    ...
    public function update_role(Request $request, $id){
        return view('roles.update_role')->with(['data' => role::find($id), 'id' => $id, 'role' => role::all()]);
    }
...
']);
    ...
    public function get_ref_detail(Request $request)
    {
        ...
        if ($request->ajax()) {
            return response()->json([
                'item_rec'  => json_decode($server_output) == '' ? json_decode($server_output) : json_decode($server_output)->data,
